...
<head>
    ...
    <title>Daftar - Smart Kas RT</title>
    ...
    <div class="flex w-full lg:w-1/2 relative overflow-hidden flex-col items-center justify-center py-10 lg:py-0"
        ...
        <div class="relative z-10 flex items-center justify-center">
            <img src="{{ asset('images/logo.png') }}" alt="Smart Kas RT Logo"
                class="w-72 h-72 lg:w-[450px] lg:h-[450px] object-contain filter drop-shadow-[0_0_20px_rgba(255,255,255,0.6)]"
        ...
        <div class="relative z-10 text-center px-8 mt-4 lg:mt-0">
            ...
            <p class="text-xs lg:text-sm mt-2" style="color:rgba(255,255,255,0.75)">Bergabunglah dengan sistem keuangan RT modern</p>
            <div class="mx-auto mt-4" style="height:2px;width:64px;background:rgba(255,255,255,0.3);border-radius:9999px"></div>
        </div>
    </div>

    <div class="w-full lg:w-1/2 flex items-center justify-center p-8 overflow-y-auto">
        <div class="w-full max-w-md py-6">
            <h2 class="text-3xl font-bold text-gray-800 mb-1">Daftar</h2>
            <p class="text-gray-500 mb-8 text-sm">Buat akun baru untuk mengakses sistem.</p>
            <form method="POST" action="{{ route('register') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="Nama Anda" required autofocus
                            class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white">
                    </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="alamat" value="{{ old('alamat') }}" placeholder="Alamat rumah"
                            class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white">
                    </div>
                </div>
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input id="pw1" type="password" name="password" placeholder="Min. 8 karakter" required
                            class="w-full pl-10 pr-12 py-3 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white">
                        <button type="button" onclick="togglePwd('pw1','e1')" class="absolute inset-y-0 right-3 flex items-center text-gray-400 hover:text-gray-600">
                            <svg id="e1" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                        </button>
                    </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input id="pw2" type="password" name="password_confirmation" placeholder="Ulangi kata sandi" required
                            class="w-full pl-10 pr-12 py-3 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white">
                        <button type="button" onclick="togglePwd('pw2','e2')" class="absolute inset-y-0 right-3 flex items-center text-gray-400 hover:text-gray-600">
                            <svg id="e2" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                        </button>
                    </div>
                </div>
                <button type="submit" class="w-full py-3 text-white font-semibold rounded-lg text-sm"
                    style="background:linear-gradient(135deg,#1E3A8A 0%,#0D9488 100%)">
                    Daftar
                </button>
            </form>
            ...
            <button type="button" onclick="loginWithGoogle(event)"
                class="w-full flex items-center justify-center gap-3 py-3 border border-gray-300 rounded-lg bg-white hover:bg-gray-50 text-sm text-gray-700 font-medium">
            ...
            <p class="text-center text-sm text-gray-500 mt-6">
                Sudah punya akun?
                <a href="{{ route('login') }}" class="font-semibold hover:underline" style="color:#0D9488">Masuk</a>
            </p>
        </div>
    </div>
    ...
    <script type="module">
        ...
        window.addEventListener('load', async () => {
            try {
                ...
                if (errEl) {
                    errEl.textContent = 'Data gagal diproses: ' + e.message;
        ...
        async function handleFirebaseLogin(idToken) {
            ...
            try {
                ...
                else { errEl.textContent = data.error || 'Autentikasi gagal.'; errEl.classList.remove('hidden'); }
            } catch (e) {
            ...
            signInWithPopup(auth, provider)
                .then(async (result) => {
                    ...
                    try {
                        ...
                        btn.disabled = false;
                        if (errEl) {
                            errEl.textContent = 'Gagal memproses pendaftaran: ' + e.message;
                            errEl.classList.remove('hidden');
                        }
                    }
                })
                .catch((error) => {
                    ...
                    if (error.code === 'auth/popup-blocked') {
                        if (errEl) {
                            errEl.innerHTML = '<b>Popup Diblokir!</b> Harap izinkan popup di browser Anda atau klik kembali tombol untuk mencoba lagi.';
                            errEl.classList.remove('hidden');
                        }
                    } else if (error.code !== 'auth/cancelled-popup-request' && error.code !== 'auth/popup-closed-by-user') {
                        if (errEl) {
                            errEl.textContent = 'Pendaftaran Gagal: ' + error.message;
                            errEl.classList.remove('hidden');
                        }
                    }
                });
